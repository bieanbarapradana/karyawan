<!DOCTYPE html>
    ...
    <div class="container">
      <div class="row">
        <div class="col-sm-12">
        <br/>
        <button class="btn btn-success" onclick="add_bajaj()"><i class="glyphicon glyphicon-plus"></i> Vehicle</button>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('bajaj/ajax_list')?>",
            "type": "POST"
        },
...
function add_bajaj()
{
  ...
  $('.modal-title').text('Add Bajaj'); // Set Title to Bootstrap modal title
}

function edit_bajaj(no_pol)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('bajaj/ajax_edit')?>/" + no_pol,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('.modal-title').text('Edit Bajaj'); // Set title to Bootstrap modal title
...
function save()
{
    $('#btnSave').text('saving...'); //Merubah value dari id btnSave menjadi saving...
    $('#btnSave').attr('disabled',true); //Mendisable (blur) btnSave
    var url;

    //Control Flow pengecekan, dengan melihat nilai save_method yg sudah diinisialisasikan dengan kondisi sbg berikut.
    if(save_method == 'add') {
        url = "<?php echo site_url('bajaj/ajax_add')?>";
    }
	else {
        url = "<?php echo site_url('bajaj/ajax_update')?>";
    }
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Terjadi Kesalahan, No Polisi Telah digunakan');
            $('#btnSave').text('save'); //change button text
            $('#btnSave').attr('disabled',false); //set button enable
...
function delete_bajaj(no_pol)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('bajaj/ajax_delete')?>/"+no_pol,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function bulk_delete()
{
    ...
    $(".data-check:checked").each(function() {
            list_no_pol.push(this.value);
    });
    if(list_no_pol.length > 0)
    {

        if(confirm('Are you sure delete this '+list_no_pol.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {no_pol:list_no_pol},
                url: "<?php echo site_url('bajaj/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Vehicle Form</h3>
            </div>
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="no_pol" placeholder="License Number" class="form-control" type="text" pattern="">
