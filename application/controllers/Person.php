...
class Person extends CI_Controller {
	...
	public function person_tetap()
	{
		$this->load->helper('url');
		$this->load->view('person_tetap_view');
	}
	...
	public function ajax_list_person_tetap()
	{
		$this->load->helper('url');

		$list = $this->person->get_datatables_tetap();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $person) 
		{
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->person->count_all_tetap(),
						"recordsFiltered" => $this->person->count_filtered_tetap(),
						"data" => $data,
				);
		//output to json format
		echo json_encode($output);
